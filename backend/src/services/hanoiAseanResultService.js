...
const HANOI_ASEAN_PROVIDER_NAME = 'Hanoi ASEAN Official';
const HANOI_ASEAN_MARKET_ID = 'hanoi_asean';
const HANOI_ASEAN_MARKET_NAME = 'ฮานอยอาเซียน';
const HANOI_ASEAN_SITE_URL = 'https://hanoiasean.com/';
const HANOI_ASEAN_RESULT_URL = 'https://hanoiasean.com/api/result';
const HANOI_ASEAN_HISTORY_URL = 'https://hanoiasean.com/api/history';
const HANOI_ASEAN_TIMEOUT_MS = Number(process.env.HANOI_ASEAN_TIMEOUT_MS || 15000);

const http = axios.create({
  timeout: HANOI_ASEAN_TIMEOUT_MS,
  headers: {
...
const buildSnapshot = ({ roundCode, results, publishedAt, rawPayload }) => {
  ...
  return {
    lotteryCode: HANOI_ASEAN_MARKET_ID,
    feedCode: HANOI_ASEAN_MARKET_ID,
    marketName: HANOI_ASEAN_MARKET_NAME,
    roundCode,
    headline: threeTop,
    firstPrize,
    threeTop,
    ...
    sourceUrl: HANOI_ASEAN_SITE_URL,
    rawPayload
  };
};

const fetchCurrentSnapshot = async () => {
  const response = await http.get(HANOI_ASEAN_RESULT_URL);
...
const fetchHistorySnapshots = async () => {
  const response = await http.get(HANOI_ASEAN_HISTORY_URL);
  ...
  return rows
    .map((row) => buildSnapshot({
      ...
      results: row?.results,
      publishedAt: parseBangkokDateTime(`${stringValue(row?.lotto_date)} 09:30:00`),
      rawPayload: row
    }))
    .filter(Boolean);
};

const fetchHanoiAseanSnapshots = async ({ limit = 10 } = {}) => {
...
const fetchLatestHanoiAseanSnapshot = async () => {
  const snapshots = await fetchHanoiAseanSnapshots({ limit: 1 });
...
module.exports = {
  ...
  HANOI_ASEAN_SITE_URL,
  fetchHanoiAseanSnapshots,
