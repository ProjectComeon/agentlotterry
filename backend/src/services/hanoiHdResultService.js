...
const HANOI_HD_PROVIDER_NAME = 'Xoso HD Official';
const HANOI_HD_MARKET_ID = 'hanoi_hd';
const HANOI_HD_MARKET_NAME = 'ฮานอย HD';
const HANOI_HD_SITE_URL = 'https://xosohd.com/';
const HANOI_HD_RESULT_URL = 'https://api.xosohd.com/result';
const HANOI_HD_HISTORY_URL = 'https://api.xosohd.com/history';
const HANOI_HD_TIMEOUT_MS = Number(process.env.HANOI_HD_TIMEOUT_MS || 15000);

const http = axios.create({
  timeout: HANOI_HD_TIMEOUT_MS,
  headers: {
...
const buildSnapshot = ({ roundCode, results, publishedAt, rawPayload }) => {
  ...
  return {
    lotteryCode: HANOI_HD_MARKET_ID,
    feedCode: HANOI_HD_MARKET_ID,
    marketName: HANOI_HD_MARKET_NAME,
    roundCode,
    headline: threeTop,
    firstPrize,
    threeTop,
    ...
    sourceUrl: HANOI_HD_SITE_URL,
    rawPayload
  };
};

const fetchCurrentSnapshot = async () => {
  const response = await http.get(HANOI_HD_RESULT_URL);
...
const fetchHistorySnapshots = async () => {
  const response = await http.get(HANOI_HD_HISTORY_URL);
  ...
  return rows
    .map((row) => buildSnapshot({
      ...
      results: row?.results,
      publishedAt: parseBangkokDateTime(`${stringValue(row?.lotto_date)} 11:30:00`),
      rawPayload: row
    }))
    .filter(Boolean);
};

const fetchHanoiHdSnapshots = async ({ limit = 10 } = {}) => {
...
const fetchLatestHanoiHdSnapshot = async () => {
  const snapshots = await fetchHanoiHdSnapshots({ limit: 1 });
