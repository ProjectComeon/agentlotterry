...
const LAOS_HD_PROVIDER_NAME = 'Lao HD Official';
const LAOS_HD_MARKET_ID = 'lao_hd';
const LAOS_HD_MARKET_NAME = '\u0e25\u0e32\u0e27 HD';
const LAOS_HD_SITE_URL = 'https://laoshd.com/';
const LAOS_HD_RESULT_URL = 'https://api.laoshd.com/api/result';
const LAOS_HD_HISTORY_URL = 'https://api.laoshd.com/api/history';
const LAOS_HD_TIMEOUT_MS = Number(process.env.LAOS_HD_TIMEOUT_MS || 15000);

const http = axios.create({
  timeout: LAOS_HD_TIMEOUT_MS,
  headers: {
...
const buildSnapshot = ({ roundCode, results, publishedAt, rawPayload }) => {
  ...
  return {
    lotteryCode: LAOS_HD_MARKET_ID,
    feedCode: LAOS_HD_MARKET_ID,
    marketName: LAOS_HD_MARKET_NAME,
    roundCode,
    headline: threeTop,
    firstPrize: digit5,
    threeTop,
    ...
    sourceUrl: LAOS_HD_SITE_URL,
    rawPayload
  };
};

const fetchCurrentSnapshot = async () => {
  const response = await http.get(LAOS_HD_RESULT_URL);
...
const fetchHistorySnapshots = async () => {
  const response = await http.get(LAOS_HD_HISTORY_URL);
  ...
  return rows
    .map((row) => buildSnapshot({
      ...
      results: row?.results,
      publishedAt: parseBangkokDateTime(`${stringValue(row?.lotto_date)} 13:45:00`),
      rawPayload: row
    }))
    .filter(Boolean);
};

const fetchLaosHdSnapshots = async ({ limit = 10 } = {}) => {
...
const fetchLatestLaosHdSnapshot = async () => {
  const snapshots = await fetchLaosHdSnapshots({ limit: 1 });
