...
const HANOI_EXTRA_PROVIDER_NAME = 'Xoso Extra Official';
const HANOI_EXTRA_MARKET_ID = 'hanoi_extra';
const HANOI_EXTRA_MARKET_NAME = 'ฮานอย Extra';
const HANOI_EXTRA_SITE_URL = 'https://xosoextra.com/';
const HANOI_EXTRA_RESULT_URL = 'https://api.xosoextra.com/result';
const HANOI_EXTRA_HISTORY_URL = 'https://api.xosoextra.com/history';
const HANOI_EXTRA_TIMEOUT_MS = Number(process.env.HANOI_EXTRA_TIMEOUT_MS || 15000);

const http = axios.create({
  timeout: HANOI_EXTRA_TIMEOUT_MS,
  headers: {
...
const buildSnapshot = ({ roundCode, results, publishedAt, rawPayload }) => {
  ...
  return {
    lotteryCode: HANOI_EXTRA_MARKET_ID,
    feedCode: HANOI_EXTRA_MARKET_ID,
    marketName: HANOI_EXTRA_MARKET_NAME,
    roundCode,
    headline: threeTop,
    firstPrize,
    threeTop,
    ...
    sourceUrl: HANOI_EXTRA_SITE_URL,
    rawPayload
  };
};

const fetchCurrentSnapshot = async () => {
  const response = await http.get(HANOI_EXTRA_RESULT_URL);
...
const fetchHistorySnapshots = async () => {
  const response = await http.get(HANOI_EXTRA_HISTORY_URL);
  ...
  return rows
    .map((row) => buildSnapshot({
      ...
      results: row?.results,
      publishedAt: parseBangkokDateTime(`${stringValue(row?.lotto_date)} 22:30:00`),
      rawPayload: row
    }))
    .filter(Boolean);
};

const fetchHanoiExtraSnapshots = async ({ limit = 10 } = {}) => {
...
const fetchLatestHanoiExtraSnapshot = async () => {
  const snapshots = await fetchHanoiExtraSnapshots({ limit: 1 });
...
module.exports = {
  ...
  HANOI_EXTRA_SITE_URL,
  fetchHanoiExtraSnapshots,
