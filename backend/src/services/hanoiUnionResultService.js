...
const HANOI_UNION_PROVIDER_NAME = 'Xoso Union Official';
const HANOI_UNION_MARKET_ID = 'hanoi_union';
const HANOI_UNION_MARKET_NAME = 'ฮานอยสามัคคี';
const HANOI_UNION_SITE_URL = 'https://xosounion.com/';
const HANOI_UNION_RESULT_URL = 'https://api.xosounion.com/api/result';
const HANOI_UNION_HISTORY_URL = 'https://api.xosounion.com/api/history';
const HANOI_UNION_TIMEOUT_MS = Number(process.env.HANOI_UNION_TIMEOUT_MS || 15000);

const http = axios.create({
  timeout: HANOI_UNION_TIMEOUT_MS,
  headers: {
...
const buildSnapshot = ({ roundCode, results, publishedAt, rawPayload }) => {
  ...
  return {
    lotteryCode: HANOI_UNION_MARKET_ID,
    feedCode: HANOI_UNION_MARKET_ID,
    marketName: HANOI_UNION_MARKET_NAME,
    roundCode,
    headline: threeTop,
    firstPrize,
    threeTop,
    ...
    sourceUrl: HANOI_UNION_SITE_URL,
    rawPayload
  };
};

const fetchCurrentSnapshot = async () => {
  const response = await http.get(HANOI_UNION_RESULT_URL);
...
const fetchHistorySnapshots = async () => {
  const response = await http.get(HANOI_UNION_HISTORY_URL);
  ...
  return rows
    .map((row) => buildSnapshot({
      ...
      results: row?.results,
      publishedAt: parseBangkokDateTime(`${stringValue(row?.lotto_date)} 17:30:00`),
      rawPayload: row
    }))
    .filter(Boolean);
};

const fetchHanoiUnionSnapshots = async ({ limit = 10 } = {}) => {
...
const fetchLatestHanoiUnionSnapshot = async () => {
  const snapshots = await fetchHanoiUnionSnapshots({ limit: 1 });
...
module.exports = {
  ...
  HANOI_UNION_SITE_URL,
  fetchHanoiUnionSnapshots,
