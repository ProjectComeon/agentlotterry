...
const HANOI_TV_PROVIDER_NAME = 'Minh Ngoc TV Official';
const HANOI_TV_MARKET_ID = 'hanoi_tv';
const HANOI_TV_MARKET_NAME = 'ฮานอย TV';
const HANOI_TV_SITE_URL = 'https://minhngoctv.com/';
const HANOI_TV_RESULT_URL = 'https://api.minhngoctv.com/result';
const HANOI_TV_TIMEOUT_MS = Number(process.env.HANOI_TV_TIMEOUT_MS || 15000);
const HANOI_TV_HISTORY_WINDOW_DAYS = Number(process.env.HANOI_TV_HISTORY_WINDOW_DAYS || 45);

const http = axios.create({
  timeout: HANOI_TV_TIMEOUT_MS,
  headers: {
...
const buildSnapshot = ({ roundCode, results, publishedAt, rawPayload }) => {
  ...
  return {
    lotteryCode: HANOI_TV_MARKET_ID,
    feedCode: HANOI_TV_MARKET_ID,
    marketName: HANOI_TV_MARKET_NAME,
    roundCode,
    headline: threeTop,
    firstPrize,
    threeTop,
    ...
    sourceUrl: HANOI_TV_SITE_URL,
    rawPayload
  };
};

const fetchSnapshotByPath = async (pathSuffix = '') => {
  const response = await http.get(`${HANOI_TV_RESULT_URL}${pathSuffix}`);
...
const fetchHistorySnapshots = async ({ limit = 10, latestRoundCode = '' } = {}) => {
  ...
  for (let offset = 1; offset <= HANOI_TV_HISTORY_WINDOW_DAYS && snapshots.length < limit; offset += 1) {
...
const fetchHanoiTvSnapshots = async ({ limit = 10 } = {}) => {
...
const fetchLatestHanoiTvSnapshot = async () => {
  const snapshots = await fetchHanoiTvSnapshots({ limit: 1 });
